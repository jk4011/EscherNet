...
exp_dir = "/data2/wlsgur4011/EscherNet/logs_6DoF/GSO3D"
...
for exp in exps:
    OUTPUT_DIR = os.path.join(exp_dir, f"N{exp}M36")
    output_NeuS = os.path.join(exp_dir, "../Neus", f"ours_GSO_T{exp}")
    os.makedirs(output_NeuS, exist_ok=True)
    obj_names = os.listdir(DATA_DIR)
    for obj_name in tqdm(obj_names):
        if obj_name == "readme.txt":
            continue
